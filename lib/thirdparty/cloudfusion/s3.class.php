...
class AmazonS3 extends CloudFusion
{
	...
	public function authenticate($bucket, $opt = null, $location = null, $redirects = 0)
	{
		...
		else
		{
			...
			if ($method == 'list_objects')
			{
				...
				{
					/* Jarrett Jordaan [jarrett@knowledgetree.com] Can't deal with spaces */
					//$request .= '&prefix=' . $prefix;
					$request .= '&prefix=' . rawurlencode($prefix);
				}

				if (isset($marker) && !empty($marker))
				{
					$request .= '&marker=' . rawurlencode($marker);
				}
	 ...
	 */
	public function delete_all_objects($bucket, $pcre = S3_PCRE_ALL, $opt = null)
	{
		// Collect all matches
		$list = $this->get_object_list($bucket, array_merge(array('pcre' => $pcre), $opt));
	 ...
	 *	Content-Type - _string_ (Optional) Content type of the file.
